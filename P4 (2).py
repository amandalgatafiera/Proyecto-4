...
# Se realiza la modulación 16-QAM:

import numpy as np

def modulador(bits, fc, pp):
    ...
    # Generando un periodo d la señal s(t)
    # pp siendo la muestras por período por cada 2 bits
    Tc = 1 / fc  # periodo, "s"
    t_periodo = np.linspace(0, Tc, pp*2)  
    pI = np.sin(2*np.pi*fc*t_periodo)
    pQ = np.cos(2*np.pi*fc*t_periodo)

    # Se inicializa la señal s(t) modulada
    t_simulacion = np.linspace(0, N*Tc, N*pp) 
    senalTx = np.zeros(t_simulacion.shape)
    moduladora = np.zeros(t_simulacion.shape) 
 
    # Formas de onda relacionado a los bits
    i=0
    for b1, b2, b3, b4 in bits2:
        if b1 == 0 and b2 == 0:
            senalTx[i*pp : (i+2)*pp] = pQ * -3
            moduladora[i*pp : (i+2)*pp] = -3
        if b1 == 0 and b2 == 1:
            senalTx[i*pp : (i+2)*pp] = pQ * -1
            moduladora[i*pp : (i+2)*pp] = -1
        if b1 == 1 and b2 == 1:
            senalTx[i*pp : (i+2)*pp] = pQ * 1
            moduladora[i*pp : (i+2)*pp] = 1
        if b1 == 1 and b2 == 0:
        ...
        if b3 == 0 and b4 == 0:
            senalTx[(i+2)*pp : (i+4)*pp] = pI * 3
            moduladora[(i+2)*pp : (i+4)*pp] = 3
        if b3 == 0 and b4 == 1:
            senalTx[(i+2)*pp : (i+4)*pp] = pI* 1
            moduladora[(i+2)*pp : (i+4)*pp] = 1
        if b3 == 1 and b4 == 1:
            senalTx[(i+2)*pp : (i+4)*pp] = pI* -1
            moduladora[(i+2)*pp : (i+4)*pp] -1
        if b3 == 1 and b4 == 0:
    ...
    P_senal_Tx = (1 / (N*Tc)) * np.trapz(pow(senalTx, 2), t_simulacion)
    print(portadora)
    return senalTx, P_senal_Tx, pI, pQ, moduladora
...
def canal_ruidoso(senalTx, Pm, SNR):
    ...
    # Potencia del ruido creado por el canal
    Pn = Pm / pow(10, SNR/10)

    # Creando ruido auditivo blanco gaussiano
    ruido = np.random.normal(0, np.sqrt(Pn), senalTx.shape)
    
    # Señal distorsionada por el canal ruidoso
    senalRx = senalTx + ruido
    return senalRx


import numpy as np

def demodulador(senalRx, pI, pQ, pp):
    ...
    return bits_Rx.astype(int), senaldemodulada
...
# La imagen a trasnsmitir se importa y se convierte
imagenTx = fuente_info('arenal.jpg')
dimensiones = imagenTx.shape

# Codificar los pixeles 
bTx = rgb_a_bit(imagenTx)
senalTx, Pm, pI, pQ, moduladora = modulador(bTx, fc, pp)
senalRx = canal_ruidoso(senalTx, Pm, SNR)

# Desmodular la señal recibida
bits_Rx, senaldemodulada = demodulador(senalRx, pI, pQ, pp)

# Se obrseva la imagen recibida
imagenRx = bits_a_rgb(bits_Rx, dimensiones)
Fig = plt.figure(figsize=(10,6))
...
# Número de errores 
errores = sum(abs(bTx - bits_Rx))
BER = errores/len(bTx)
print('{} errores, para un BER de {:0.4f}.'.format(errores, BER))

# Imagen transmitida
ax = Fig.add_subplot(1, 2, 1)
imgplot = plt.imshow(imagenTx)
ax.set_title('Transmitido')

# Imagen recuperada
ax = Fig.add_subplot(1, 2, 2)
imgplot = plt.imshow(imagenRx)
ax.set_title('Recuperado')
Fig.tight_layout()

plt.imshow(imagenRx)

# Cambio entre las señales
fig, (ax1, ax2, ax3, ax4) = plt.subplots(nrows=4, sharex=True, figsize=(14, 7))
...
# Señal modulada por BPSK
ax2.plot(senalTx[0:600], color='g', lw=2) 
ax2.set_ylabel('$s(t)$')

# Señal modulada al dejar el canal
ax3.plot(senalRx[0:600], color='b', lw=2) 
ax3.set_ylabel('$s(t) + n(t)$')

# Señal demodulada
ax4.plot(senaldemodulada[0:600], color='m', lw=2) 
ax4.set_ylabel('$b^{\prime}(t)$')
ax4.set_xlabel('$t$ / milisegundos')
fig.tight_layout()
plt.show()
